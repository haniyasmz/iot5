...
def init_connection():
    return psycopg2.connect(user=st.secrets.postgres.user, password=st.secrets.postgres.password, host=st.secrets.postgres.host, db=st.secrets.postgres.dbname, port=st.secrets.postgres.port)
